...
- Latest benchmark evidence: `embedding-retrieval-benchmark-2026-04-02-214418.json`
- Best quality now: `openai / text-embedding-3-large / 768d`
- Best balanced default: `openai / text-embedding-3-small / 768d`
- Best local-first path: `ollama / embeddinggemma / 768d`
- Do not in-place replace the current index with `gemini-embedding-2-preview`

## Ranked strategies

### OpenAI text-embedding-3-large (768d request)

- Tier: `best_quality_now`
...
- Why it fits Wiii: OpenAI positions text-embedding-3-large as its most capable embedding model for English and non-English retrieval. In Wiii's latest direct benchmark, it also delivered the fastest query embedding measurement among the benchmarked paths while keeping hybrid retrieval healthy.
- Tradeoffs: Higher cost than text-embedding-3-small and still cloud-dependent. The current evidence is strong, but it should still be rechecked over multiple benchmark rounds before becoming the only default.
- Adoption path: Use for quality-first retrieval lanes, premium orgs, or controlled bake-offs. If later rounds stay consistent, promote it as the quality tier while keeping a cheaper balanced tier beside it.
- Measured query embed latency: `540.78 ms`
- Measured hybrid search latency: `180.08 ms`
- Sources:
  - https://developers.openai.com/api/docs/models/text-embedding-3-large

### OpenAI text-embedding-3-small (768d request)

- Tier: `balanced_default_now`
...
- Why it fits Wiii: OpenAI positions text-embedding-3-small as the default small embedding model, with materially lower cost than text-embedding-3-large. On Wiii's latest live benchmark, it completed query embedding, semantic context retrieval, and hybrid search successfully against the current 768d index.
- Tradeoffs: Cloud-only dependency and, in the latest benchmark, no longer the fastest option. It remains valuable because its cost profile is much easier to carry as a broad default.
- Adoption path: Keep as the default balanced tier and as the cloud safety net behind local-first or quality-first choices.
- Measured query embed latency: `2886.45 ms`
- Measured hybrid search latency: `192.86 ms`
- Sources:
...
- Measured query embed latency: `2280.71 ms`
- Measured hybrid search latency: `525.65 ms`
- Sources:
...
- Query embedding: `2886.45 ms`, ok=`True`
- Document embedding: `180.04 ms`, ok=`True`
- Semantic context: `247.09 ms`, memories=`1`, facts=`1`
- Hybrid search: `192.86 ms`, results=`2`, method=`hybrid`
- Observations: provider_promoted_to_openai

### ollama_local_first

- Active backend: `ollama / embeddinggemma / 768d`
- Query embedding: `2280.71 ms`, ok=`True`
- Document embedding: `263.01 ms`, ok=`True`
- Semantic context: `356.24 ms`, memories=`1`, facts=`1`
- Hybrid search: `525.65 ms`, results=`2`, method=`hybrid`

### openai_large_768

- Active backend: `openai / text-embedding-3-large / 768d`
- Query embedding: `540.78 ms`, ok=`True`
- Document embedding: `165.71 ms`, ok=`True`
- Semantic context: `500.77 ms`, memories=`1`, facts=`1`
- Hybrid search: `180.08 ms`, results=`2`, method=`hybrid`
