...
# =============================================================================
# TEST 5: PERSONALITY EVALUATOR
# =============================================================================

class TestPersonalityEvaluator:
    """Sprint 115 Improvement #5: Personality drift detection."""

    def test_evaluator_loads_from_yaml(self):
        """Should load rules from wiii_identity.yaml."""
        from app.engine.evaluation.personality_eval import PersonalityEvaluator
        ev = PersonalityEvaluator()
        assert len(ev._avoids) > 0
        assert len(ev._anticharacter) > 0
        assert len(ev._catchphrases) > 0

    def test_good_response_high_score(self):
        """Well-formed Vietnamese response should score high."""
        from app.engine.evaluation.personality_eval import PersonalityEvaluator
        ev = PersonalityEvaluator()
        score = ev.evaluate(
            "Hay quá~ Rule 15 nói về tình huống cắt hướng. "
            "Đơn giản là tàu thấy tàu kia bên mạn phải thì nhường. "
            "Hiểu ý mình không? 😄"
        )
        assert score.overall >= 0.7
        assert not score.is_drifting

    def test_robot_response_low_score(self):
        """Robotic/corporate AI response should score lower."""
        from app.engine.evaluation.personality_eval import PersonalityEvaluator
        ev = PersonalityEvaluator()
        score = ev.evaluate(
            "Tôi là trợ lý AI. Tôi có thể giúp bạn với câu hỏi này. "
            "Như chúng ta đã biết, quy tắc 15 quy định rằng..."
        )
        assert score.anticharacter_violations > 0

    def test_greeting_leak_detected(self):
        """Greeting in follow-up should be detected."""
        from app.engine.evaluation.personality_eval import PersonalityEvaluator
        ev = PersonalityEvaluator()
        score = ev.evaluate(
            "Chào bạn! Rất vui được hỗ trợ. Rule 15 quy định...",
            is_follow_up=True,
        )
        assert score.greeting_leak is True

    def test_no_greeting_leak_first_message(self):
        """Greeting in first message should NOT be flagged."""
        from app.engine.evaluation.personality_eval import PersonalityEvaluator
        ev = PersonalityEvaluator()
        score = ev.evaluate(
        ...
        assert score.greeting_leak is False

    def test_emoji_density(self):
        """Should calculate emoji density correctly."""
        from app.engine.evaluation.personality_eval import PersonalityEvaluator
        ev = PersonalityEvaluator()
        score = ev.evaluate("Hay quá~ 😄✨⚓ Rất vui!")
        assert score.emoji_density > 0

    def test_catchphrase_count(self):
        """Should count catchphrases."""
        from app.engine.evaluation.personality_eval import PersonalityEvaluator
        ev = PersonalityEvaluator()
        score = ev.evaluate(
            "Hay quá~ Hmm, để mình nghĩ xem... Hiểu ý mình không? 😄"
        )
        assert score.catchphrase_count >= 2

    def test_empty_response(self):
        """Empty response should return default score."""
        from app.engine.evaluation.personality_eval import PersonalityEvaluator
        ev = PersonalityEvaluator()
        score = ev.evaluate("")
        assert score.overall == 1.0  # No violations

    def test_is_drifting_threshold(self):
        """is_drifting should be True when overall < 0.6."""
        from app.engine.evaluation.personality_eval import PersonalityScore
        score = PersonalityScore(
            avoid_violations=3,
            anticharacter_violations=2,
            greeting_leak=True,
        )
        assert score.is_drifting

    def test_personality_score_defaults(self):
        """Default PersonalityScore should have high overall."""
        from app.engine.evaluation.personality_eval import PersonalityScore
        score = PersonalityScore()
        assert score.overall == 1.0
        assert not score.is_drifting

    def test_voice_consistency_vietnamese(self):
        """Vietnamese text should have high voice_consistency."""
        from app.engine.evaluation.personality_eval import PersonalityEvaluator
        ev = PersonalityEvaluator()
        score = ev.evaluate("Đây là câu trả lời bằng tiếng Việt có dấu đầy đủ.")
        assert score.voice_consistency >= 0.7

    def test_voice_consistency_english(self):
        """Pure English text should have low voice_consistency."""
        from app.engine.evaluation.personality_eval import PersonalityEvaluator
        ev = PersonalityEvaluator()
        score = ev.evaluate("This is a response in English without Vietnamese characters.")
        assert score.voice_consistency < 0.5

    def test_formal_language_detection(self):
        """Formal language should be detected."""
        from app.engine.evaluation.personality_eval import PersonalityEvaluator
        ev = PersonalityEvaluator()
        score = ev.evaluate("Kính gửi quý vị, hân hạnh được trân trọng phục vụ.")
        assert score.formal_language_score > 0

    def test_avoid_violation_starts_with_a(self):
        """Response starting with 'À,' should be detected."""
        from app.engine.evaluation.personality_eval import PersonalityEvaluator
        ev = PersonalityEvaluator()
        score = ev.evaluate("À, câu hỏi hay lắm! Tôi xin trả lời như sau...")
        assert score.avoid_violations >= 1

    def test_singleton(self):
        """get_personality_evaluator should return singleton."""
        from app.engine.evaluation.personality_eval import get_personality_evaluator
        e1 = get_personality_evaluator()
        e2 = get_personality_evaluator()
        assert e1 is e2
...
# =============================================================================
# TEST: INTEGRATION — Anticharacter + Evaluator
# =============================================================================

class TestAnticharacterEvaluatorIntegration:
    """Sprint 115: Evaluator uses anticharacter from YAML."""

    def test_evaluator_loads_anticharacter(self):
        """PersonalityEvaluator should load anticharacter items."""
        from app.engine.evaluation.personality_eval import PersonalityEvaluator
        ev = PersonalityEvaluator()
        assert len(ev._anticharacter) == 10

    def test_evaluator_detects_anticharacter_pattern(self):
        """Evaluator should detect anticharacter violations."""
        from app.engine.evaluation.personality_eval import PersonalityEvaluator
        ev = PersonalityEvaluator()
        # This response matches "nói giọng máy móc" anticharacter
        score = ev.evaluate(
            "Tôi là trợ lý AI. Như chúng ta đã biết, xin chào quý vị."
        )
        assert score.anticharacter_violations >= 2
